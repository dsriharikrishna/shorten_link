...
const CampaignSource = () => {
  ...
  // Memoized safe array, filtered for sources only
  const data = useMemo(
    () =>
      Array.isArray(rawData)
        ? rawData.filter((item) => item.campaign_source_name)
        : [],
    [rawData]
  );
  ...
  useEffect(() => {
    ...
    setRows(
      paginated.map((item) => ({
        id: item.id,
        tag: item.campaign_source_tag,
        source: item.campaign_source_name,
  ...
  const handleConfirmDelete = useCallback(async () => {
    ...
    try {
      ...
      if (response?.details) {
        toast.success(
          response?.details?.message || "Campaign source deleted successfully"
        );
        handleDeleteClose();
        setTimeout(() => {
  ...
  return (
    ...
    >
      ...
      >
        ...
        >
          {isMobile ? "Add Source" : "Add Campaign Source"}
        </Button>
      </Box>
